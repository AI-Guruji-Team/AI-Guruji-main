...
import { getQuestionsByCategory } from '../utils/questionLoader';
...
const NonConventionalAssessment: React.FC = () => {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        const loadedQuestions = getQuestionsByCategory('nonconventional');
  ...
  const saveResults = async () => {
    ...
    localStorage.setItem('nonConventionalTestScores', JSON.stringify(scoresWithTotal));
    
    // Log results to console since we don't have Supabase integration in this example
    console.log('Assessment results:', {
      user_id: user?.id || 'anonymous',
      assessment_type: 'nonconventional',
      scores: scores,
  ...
  if (isCompleted) {
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="mb-6">
          <h3 className="text-xl font-semibold mb-4">Your Top Non-Conventional Career Matches:</h3>
...
export default NonConventionalAssessment; 