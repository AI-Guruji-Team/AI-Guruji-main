...
const GenericAssessment: React.FC<GenericAssessmentProps> = ({ assessmentType }) => {
  ...
  const { user } = useAuthStore(); // Kept for future backend integration
  const navigate = useNavigate();

  useEffect(() => {
    // Load questions based on assessment type
    console.log(`Loading questions for assessment type: ${assessmentType.toLowerCase()}`);
    const loadedQuestions = getQuestionsByCategory(assessmentType.toLowerCase());
    console.log(`Loaded ${loadedQuestions.length} questions for ${assessmentType}`);
  ...
  const saveScoresToLocalStorage = () => {
    ...
    // Make sure we're using the correct key format for each assessment type
    let storageKey = '';
    if (assessmentType.toLowerCase() === 'nonconventional') {
      storageKey = 'nonConventionalTestScores'; // Match the key used in Reports.tsx
    } else {
      storageKey = `${assessmentType.toLowerCase()}TestScores`;
    }
    
    localStorage.setItem(storageKey, JSON.stringify(scoresObject));
    console.log(`Saved ${assessmentType} scores to localStorage with key: ${storageKey}`, scoresObject);
  };
